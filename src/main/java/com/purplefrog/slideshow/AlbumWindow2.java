package com.purplefrog.slideshow;

import java.awt.*;
import java.net.*;
import java.util.*;

/**
 * This represents a span of ImageSets.
 *
 * Created with IntelliJ IDEA.
 * User: thoth
 * Date: 3/11/13
 * Time: 2:46 PM
 * To change this template use File | Settings | File Templates.
 */
public class AlbumWindow2
{
    ImageSet2[] window;
    //URL[] urls = null;

    Map<URL, ImageSet2> perURL = new HashMap<URL, ImageSet2>();

    public AlbumWindow2(int count)
    {
        window = new ImageSet2[count];
    }

    public ImageSet2 getIP(int albumOff)
    {
        return window[albumOff];
    }

    public void clearDisplayImages()
    {
        for (ImageSet2 imageSet : window) {
            if (imageSet != null)
                imageSet .clearDisplayImages();
        }
    }

    public int getCount()
    {
        return window.length;
    }

    public URL URLforImage(Image img)
    {
        int idx = indexForImage(img);
        return window[idx].url;
    }

    public void setURLs(URL[] newURLs)
    {
        Map<URL, ImageSet2> oldMap = new HashMap<URL, ImageSet2>(perURL);

        Map<URL, ImageSet2> newMap = new HashMap<URL, ImageSet2>();

        ImageSet2[] newWindow = new ImageSet2[newURLs.length];

        for (int i = 0; i < newURLs.length; i++) {
            URL url = newURLs[i];

            ImageSet2 recovered;
            if (url == null) {
                recovered = new ImageSet2(url);
            } else {
                recovered = oldMap.remove(url);
                if (recovered == null) {
                    recovered = new ImageSet2(url);
                }
                newMap.put(url, recovered);
            }

            newWindow[i] = recovered;
        }

        window = newWindow;
        perURL = newMap;

        // discard unrecovered images
        for (ImageSet2 imageSet : oldMap.values()) {
            imageSet.flush();
        }
    }

    public void resetBrokenImages(SlideShowPane ssp)
    {
        for (ImageSet2 imageSet : window) {
            imageSet.resetBrokenImages(ssp);
        }
    }

    public int indexForImage(Image img)
    {
        for (int i = 0; i < window.length; i++) {
            ImageSet2 imageSet = window[i];
            if (imageSet.matchesImage(img))
                return i;
        }

        return -1;
    }
}
