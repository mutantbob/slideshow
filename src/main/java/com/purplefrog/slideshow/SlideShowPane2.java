    ...
    implements MouseListener, MouseMotionListener
{
        ...
        implements Runnable
    {
            ...
            throws IOException, ImageReadException
        {
            ...
            int centerIdx=beforeCols*otherRows;

            ImageSet.Parameters p2 = getSizeParameters();

            // Quick pass to make thumbnails from already loaded base images
            // Skip anything for which we do not have the base image yet
            for (int i=0; i<n; i++) {
                curr = images.getIP(i);
                if (curr.baseImg!=null) {
                    if (null != curr.url && null==curr.img) {
                        curr.loadThumbnailImage(p2);
                        int k = images.indexForImage(curr.img);
                        repaintBox(k);
                        return;
                    }
                }
            ...
            curr = images.getIP(centerIdx);
            // let's get the center version of the center image
            if (null != curr.url && curr.bigImg==null) {
                curr.loadCenterImage(p2);
                repaintBox(centerIdx);

                //imgStatusLED(i, Color.green);
                return;
            }

            // now let's load thumbnails for all the other image slots
            for (int j=0; j<n; j++) {
                int i = butterfly(j);
                //System.out.println("butterfly("+j+") = "+i);

                curr = images.getIP(i);
                if (null != curr.url && null == curr.img) {
                    curr.loadThumbnailImage(p2);
                    synchronized (SlideShowPane2.this) {
                        int k = images.indexForImage(curr.img);
                        repaintBox(k);
                    }
                    return;
    ...
    public int butterfly(int j)
    {
        int centerIdx = beforeCols * otherRows;
        if (j < afterCols*otherRows)
            return j+ centerIdx +1;
        j -= afterCols*otherRows;
        if (j<centerIdx)
            return centerIdx-j-1;
    ...
    public SlideShowPane2(List<URL> urls,
                         ...
                         int afterCols /*int afterRows,*/)
    {
        ...
        images = new AlbumWindow2(countOnScreen());
    ...
    private ImageSet.Parameters getSizeParameters()
    {
        Dimension d = getSize();

        return new ImageSet.Parameters(smallWidth, smallHeight, centerWidth, d.height, null);
    }
...
*/
    ...
    public void paint_(Graphics g)
    {
        ...
        for (int i=0; i<n; i++) {
            ...
            //System.out.println("slot "+i+" hit clip? "+g.hitClip(curr.x, curr.y, w2, h2));

            if (g.hitClip(curr.x, curr.y, w2, h2)) {

                if (validUrlsOff(urlsOff) && qq.url != null) {
                    Image img = qq.dispImg(isBig);
                    if (img==null)
                        img = qq.baseImg;
                    if (null != img)
                        drawImageInRectangle(g, img, curr.x, curr.y, w2, h2);
...
//        g.setColor(getBackground());
    ...
    protected void drawMag(Graphics g) {
        ...
        ImageSet2	bigun = images.getIP(offOfBig);
